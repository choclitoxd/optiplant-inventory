...
export const BranchStockView = () => {
  ...
  // Derivamos los datos combinando Catálogo e Inventario
  const stockList = products.map(p => {
    const inv = inventories.find(i => i.productId === p.id);
    return {
      product: p,
      stock: inv?.stock || 0,
      threshold: inv?.minStockThreshold || 0
    };
  });
  ...
  return (
    <div className="bg-white/70 backdrop-blur-md rounded-2xl shadow-sm border border-slate-200 p-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <h2 className="text-2xl font-semibold text-slate-800 tracking-tight">Control de Stock</h2>
        <select 
