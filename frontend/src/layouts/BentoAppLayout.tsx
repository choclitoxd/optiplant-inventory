...
export const BentoAppLayout: React.FC = () => {
  ...
  return (
    <div className="h-screen w-full bg-slate-50 font-sans text-slate-800 selection:bg-indigo-200/50 flex overflow-hidden">
      ...
      <aside 
        className={`bg-slate-900 border-r border-slate-800 flex flex-col justify-between transition-all duration-300 ease-in-out relative z-20 h-full ${
      ...
      >
        ...
        <div className="w-full flex flex-col items-center justify-start py-6 h-full overflow-hidden">
          
          {/* Logo */}
          <div className={`flex items-center gap-3 px-4 mb-8 h-10 w-full shrink-0 ${isSidebarOpen ? 'justify-start' : 'justify-center'}`}>
          ...
          {/* Navegación */}
          <nav className="flex flex-col w-full px-3 gap-2 overflow-y-auto custom-scrollbar flex-1">
            {navItems.map((item) => {
              ...
              return (
                <NavLink
                  ...
                  to={item.path}
                  className={({ isActive }) => 
                    `group relative flex items-center p-3 rounded-xl text-[13px] font-bold transition-all duration-200 shrink-0 ${
                      isActive 
      ...
      {/* Contenido Principal */}
      <main className="flex-1 flex flex-col min-w-0 transition-all duration-300 relative h-full overflow-y-auto custom-scrollbar">
        <div className="p-6 md:p-8 flex-1 flex flex-col w-full min-h-max">
